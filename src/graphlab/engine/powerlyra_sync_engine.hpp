...
namespace graphlab {
  ...
  private:
    ...
    /**
     * Determine if there are post_round() function to be called
     */
    bool post_round_flag;
    ...
    */
    ...
    /**
     * \brief The time spends on postround phase.
     */
    double postround_time;
    ...
    /**
     * \brief The number of `send_activs` communications 
     * in this iteration.
     */
    atomic<size_t> num_send_activs;
    /**
     * \brief The number of `send_updates` communications 
     * in this iteration.
     */
    atomic<size_t> num_send_updates;
    /**
     * \brief The number of `send_updates_activs` communications 
     * in this iteration.
     */
    atomic<size_t> num_send_updates_activs;
    /**
     * \brief The number of `send_accum` communications 
     * in this iteration.
     */
    atomic<size_t> num_send_accums;
    /**
     * \brief The number of `send_message` communications 
     * in this iteration.
     */
    atomic<size_t> num_send_messages;
    ...
    /**
     * \brief Indicating all vertices to have their values reset.
     */
    dense_bitset reset_all;
  ...
  public:
    ...
    void signal_source(vertex_id_type vertex_id,
  ...
  private:
    ...
    /**
     * \brief Execute the \ref graphlab::ivertex_program::reset
     * function on all vertices for single-instance testing.
     * 
     * @param thread_id the thread to run this as which determines
     * which vertices to process.
     */
    void execute_resets(size_t thread_id);

    void execute_postround(size_t thread_id);

    void execute_source_flush(size_t thread_id);

    void execute_source_init(size_t thread_id);

    void execute_source_scatter(size_t thread_id);
      ...
      if (opt == "max_iterations") {
        ...
        if (rmi.procid() == 0)
          logstream(LOG_EMPH) << "Engine Option: sched_allv = "
            << sched_allv << std::endl;
      } else if (opt == "post_round") {
        opts.get_engine_args().get_option("post_round", post_round_flag);
        if (rmi.procid() == 0)
          logstream(LOG_EMPH) << "Engine Option: post_round = "
            << post_round_flag << std::endl;
      } else {
  ...
  void powerlyra_sync_engine<VertexProgram>:: init() {
    ...
    reset_all.fill();
#ifdef COMM_STATS
      num_send_messages = num_send_accums = num_send_updates = 
        num_send_updates_activs = num_send_activs = 0;
#endif  // COMM_STATS
  ...
  template<typename VertexProgram> void powerlyra_sync_engine<VertexProgram>::reset() {
    if (vlocks.size() != graph.num_local_vertices())
      resize();
    rmi.barrier();
    ...
    // rely on rmi.barrier() within run_synchronous
    run_synchronous( &powerlyra_sync_engine::execute_resets );
  }

  template<typename VertexProgram> void powerlyra_sync_engine<VertexProgram>::signal_source(vertex_id_type vertex_id, const message_type& message) {
    if (vlocks.size() != graph.num_local_vertices())
      resize();
    rmi.barrier();

    if (graph.is_master(vertex_id)) {
      const vertex_type vertex  = graph.vertex(vertex_id);
      const lvid_type lvid = vertex.local_id();
      vlocks[lvid].lock();
    ...
    // flush source vertex signal messages
    run_synchronous( &powerlyra_sync_engine::execute_source_flush );

    // call execute_source_init
    run_synchronous( &powerlyra_sync_engine::execute_source_init );
    has_message.clear();

    // call execute_source_scatter
    run_synchronous( &powerlyra_sync_engine::execute_source_scatter );
...
// #endif  // COMM_STATS
    ...
    while(iteration_counter < max_iterations && !force_abort ) {
      ...
      if(rmi.procid() == 0) {
        logstream(LOG_EMPH) << rmi.procid() << ": Starting iteration: " << iteration_counter << std::endl;
      ...
      has_gather_accum.clear();
      reset_all.fill(); // see if this is necessary
...
// #endif  // COMM_STATS
...
#ifdef ENGINE_DEBUG_PRINT
      if (rmi.procid() == 0) {
        logstream(LOG_EMPH) << "\tActive vertices: " << total_active_vertices << std::endl;
      }
#else
      if (rmi.procid() == 0 && print_this_round) {
        logstream(LOG_EMPH)
      ...
      if (post_round_flag) {
        ...
        bk_ti.start();
#endif
        run_synchronous( &powerlyra_sync_engine::execute_postround );
#ifdef TUNING
        postround_time += bk_ti.current_time();
#endif
      }
...
#ifdef COMM_STATS
    global_completed = num_send_accums;
    rmi.all_reduce(global_completed);
    num_send_accums = global_completed;

    global_completed = num_send_activs;
    rmi.all_reduce(global_completed);
    num_send_activs = global_completed;

    global_completed = num_send_messages;
    rmi.all_reduce(global_completed);
    num_send_messages = global_completed;

    global_completed = num_send_updates;
    rmi.all_reduce(global_completed);
    num_send_updates = global_completed;

    global_completed = num_send_updates_activs;
    rmi.all_reduce(global_completed);
    num_send_updates_activs = global_completed;
#endif  // COMM_STATS

    if (rmi.procid() == 0) {
      logstream(LOG_EMPH) << "Compute Balance: ";
      for (size_t i = 0;i < all_compute_time_vec.size(); ++i) {
        logstream(LOG_EMPH) << all_compute_time_vec[i] << " ";
      }
      logstream(LOG_EMPH) << std::endl;
#ifdef TUNING
      logstream(LOG_EMPH) << "Total Calls(G|A|S): " 
                          << completed_gathers.value << "|" 
                          << completed_applys.value << "|"
                          << completed_scatters.value 
                          << std::endl;
...
#ifdef COMM_STATS
      logstream(LOG_EMPH) << std::endl;
      logstream(LOG_EMPH) << "COMM_STATS(Accum|Active|Msg|Update|Update-Active):"
                          << num_send_accums.value << "|"
                          << num_send_activs.value << "|"
                          << num_send_messages.value << "|"
                          << num_send_updates.value << "|"
                          << num_send_updates_activs.value
                          << std::endl;
#endif // COMM_STATS
    }
  ...
  template<typename VertexProgram>
  void powerlyra_sync_engine<VertexProgram>::
  execute_resets(const size_t thread_id) {
    context_type context(*this, graph);
    // size_t vcount = 0;
    ...
    while (1) {
      ...
      lvid_type lvid_block_start =
                  shared_lvid_counter.inc_ret_last(8 * sizeof(size_t));
      if (lvid_block_start >= graph.num_local_vertices()) break;
      // get the bit field indicating all vertices to reset
      size_t lvid_bit_block = reset_all.containing_word(lvid_block_start);
      if (lvid_bit_block == 0) continue;
      local_bitset.clear();
      local_bitset.initialize_from_mem(&lvid_bit_block, sizeof(size_t));
      foreach(size_t lvid_block_offset, local_bitset) {
        ...
        // both master and mirrors should be reset
        vertex_type vertex(graph.l_vertex(lvid));
        // call reset function in vertex_programs
        vertex_programs[lvid].reset(context, vertex);
        // // synchronize the changed vertex data with all mirrors
        // sync_vertex_data(lvid, thread_id);

        // clear vertex_program
        vertex_programs[lvid] = vertex_program_type();
      }
    }
  } // end of execute_resets

  template<typename VertexProgram>
  void powerlyra_sync_engine<VertexProgram>::
  execute_postround(const size_t thread_id) {
    context_type context(*this, graph);
    // size_t vcount = 0;
    ...
    while (1) {
      ...
      foreach(size_t lvid_block_offset, local_bitset) {
        ...
        // both master and mirrors should be reset
        vertex_type vertex(graph.l_vertex(lvid));
        // call postround function in vertex_programs
        vertex_programs[lvid].postround(context, vertex);
        // // synchronize the changed vertex data with all mirrors
        // sync_vertex_data(lvid, thread_id);

        // we are done so clear vertex_program
        vertex_programs[lvid] = vertex_program_type();
      }
    }
  } // end of execute_postround

  template<typename VertexProgram>
  void powerlyra_sync_engine<VertexProgram>::
  execute_source_flush(const size_t thread_id) {
    context_type context(*this, graph);
    fixed_dense_bitset<8 * sizeof(size_t)> local_bitset; // a word-size = 64 bit
    ...
    thread_barrier.wait();
    recv_messages();
  } // end of execute_source_flush

  template<typename VertexProgram>
  void powerlyra_sync_engine<VertexProgram>::
  execute_source_init(const size_t thread_id) {
    context_type context(*this, graph);
    fixed_dense_bitset<8 * sizeof(size_t)> local_bitset;  // allocate a word size = 64bits
    const size_t TRY_RECV_MOD = 1000;
    size_t vcount = 0;
    ...
    while (1) {
      ...
      foreach(size_t lvid_block_offset, local_bitset) {
        ...
        ASSERT_TRUE(graph.l_is_master(lvid));
        vertex_type vertex(graph.l_vertex(lvid));
        vertex_programs[lvid].source_init(context, vertex, messages[lvid]);
        // clear the message to save some memory
        messages[lvid] = message_type();
        // determine if a scatter operation is needed
        const vertex_program_type& const_vprog = vertex_programs[lvid];
        const vertex_type const_vertex = vertex;
    ...
    thread_barrier.wait();
    recv_updates_activs(); recv_updates();
  } // end of execute_source_init

  template<typename VertexProgram>
  void powerlyra_sync_engine<VertexProgram>::
  execute_source_scatter(const size_t thread_id) {
    context_type context(*this, graph);
    fixed_dense_bitset<8 * sizeof(size_t)> local_bitset; // allocate a word size = 64 bits
    size_t nscatter_inc = 0;
