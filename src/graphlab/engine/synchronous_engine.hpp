...
// #define ENGINE_DEBUG_PRINT  // debug print
...
namespace graphlab {
  ...
  private:
    ...
    */
     ...
     */
    dense_bitset reset_all; // added by x
      ...
      if (opt == "max_iterations") {
        ...
        if (rmi.procid() == 0)
          logstream(LOG_EMPH) << "Engine Option: sched_allv = "
            << sched_allv << std::endl;
      } else if (opt == "post_round") { // added by x
  ...
  void synchronous_engine<VertexProgram>:: resize() {
    ...
    reset_all.resize(graph.num_local_vertices()); // added by x
  ...
  // added by x
  template<typename VertexProgram> void synchronous_engine<VertexProgram>::reset() {
    if (vlocks.size() != graph.num_local_vertices())
      resize();
    rmi.barrier();
    ...
    run_synchronous( &synchronous_engine::execute_resets );
  }

  template<typename VertexProgram> void synchronous_engine<VertexProgram>::signal_source(vertex_id_type vertex_id,
  const message_type& message) {
    if (vlocks.size() != graph.num_local_vertices())
      resize();
    rmi.barrier();
    ...
    /**
     * TODO: 
     * This should function be `apply + scatter` combined
     */
    ...
    run_synchronous( &synchronous_engine::execute_source_flush );

    // call execute_source_init
    run_synchronous( &synchronous_engine::execute_source_init );
    has_message.clear();

    // call execute_source_scatter
    run_synchronous( &synchronous_engine::execute_source_scatter );
...
#ifdef ENGINE_DEBUG_PRINT
        if (rmi.procid() == 0)
          logstream(LOG_EMPH) << "Call PostRound()" << std::endl;
#endif  // ENGINE_DEBUG_PRINT
        run_synchronous( &synchronous_engine::execute_postround );
      }
  ...
  execute_source_init(const size_t thread_id) {
    ...
    } // end of loop over vertices to run source_init
    vprog_exchange.partial_flush();
    vdata_exchange.partial_flush();
      // Finish sending and receiving all changes due to apply operations
    thread_barrier.wait();
  ...
  execute_resets(const size_t thread_id) {
    ...
    // vprog_exchange.partial_flush();
    // vdata_exchange.partial_flush();
    // Finish sending and receiving all changes due to reset operations
    // thread_barrier.wait();
    // if(thread_id == 0) { 
    //   vprog_exchange.flush(); vdata_exchange.flush(); 
    // }
