...
#pragma omp parallel
    ...
    void save_format(const std::string& prefix, const std::string& format,
      ...
      } else if (format == "etuple") {
        save(prefix, builtin_parsers::etuple_writer<distributed_graph>(),
             gzip, false, true, files_per_machine);
      } else if (format == "wtuple") {
        save(prefix, builtin_parsers::wtuple_writer<distributed_graph>(),
             gzip, false, true, files_per_machine);
...
#pragma omp parallel for
    ...
    void load_format(const std::string& path, const std::string& format) {
      ...
      if (format == "snap") {
        ...
        line_parser = builtin_parsers::etuple_parser<distributed_graph>;
        load(path, line_parser);
      } else if (format == "wtuple") {
        line_parser = builtin_parsers::wtuple_parser<distributed_graph>;
        load(path, line_parser);
      } else if (format == "msbfs") {
        line_parser = builtin_parsers::msbfs_parser<distributed_graph>;
        load(path, line_parser);
      } else if (format == "adj") {
    ...
    /**
     *  \brief load a graph with a unweighted format. Must be called on all
     *  machines simultaneously.
     *
     *  The supported graph formats are described in \ref graph_formats.
     */
    void load_uw_format(const std::string& path, const std::string& format) {
      line_parser_type line_parser;
      if (format == "snap") {
        line_parser = builtin_parsers::snap_parser<distributed_graph>;
        load(path, line_parser);
