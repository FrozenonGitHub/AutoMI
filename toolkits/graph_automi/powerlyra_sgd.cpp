...
struct vertex_data {
  ...
  /**
   * \brief Randomize the latent pvec.
   */
  void randomize() { 
    pvec.resize(NLATENT);
    pvec.setRandom();
...
class msg_type {
  public:
    /**
     * \brief Stores the current sum of errors.
     */
    vec_type pvec;

    msg_type() { }

    msg_type(const vec_type& X) : pvec(X) { }
...
class vertex_program :
	public graphlab::ivertex_program<graph_type, msg_type>, public graphlab::IS_POD_TYPE {
		public:
      ...
      msg_type gather(icontext_type& context, const vertex_type& vertex, 
        ...
        vertex_type other_vertex = get_other_vertex(edge, my_vertex);
        ...
        if (edge.data().role == edge_data::TRAIN) {
          delta = err * other_vertex.data().pvec;
        } else {
          delta = vec_type::Zero(vertex_data::NLATENT);
        }
        ...
        vec_type temp = msg_accum.pvec - LAMBDA * vdata.pvec;
        vdata.pvec += GAMMA * temp;
				++vdata.nupdates;
			} // end of apply
...
double extract_l2_error(const graph_type::edge_type & edge) {
	...
	pred = std::min(vertex_program::MAXVAL, pred);
	pred = std::max(vertex_program::MINVAL, pred);
	double rmse = (edge.data().obs - pred) * (edge.data().obs - pred);
	assert(rmse <= pow(vertex_program::MAXVAL-vertex_program::MINVAL,2));
	return rmse;
...
int main(int argc, char** argv) {
  ...
  for(size_t i = 0; i < sources.size(); ++i) {
    engine.signal_source(sources[i], msg_type());
    timer.start();
    engine.start();
