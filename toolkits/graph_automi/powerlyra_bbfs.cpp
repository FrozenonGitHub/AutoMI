...
public:
    ...
    void apply(icontext_type& context, vertex_type& vertex,
        ...
        changed = msg_accum.ans & (!vertex.data().ans);;
        vertex.data().ans = vertex.data().ans | msg_accum.ans;
    }
...
int main(int argc, char** argv) {
  ...
  graphlab::command_line_options 
    clopts("PowerLyra BFS Algorithm.");
  ...
  for(size_t i = 0; i < sources.size(); ++i) {
    engine.signal_source(sources[i], msg_type(true));
    timer.start();
    engine.start();
