 ...
 */
typedef bool ans_type;
ans_type default_ans = false;
...
struct vertex_data {
    ...
    explicit vertex_data(const std::vector<ans_type>& ans_in) : ans(ans_in) { }
...
/**
 * \brief No edge data for unweighted graph.
 */
...
struct bit_str {
    ...
    bit_str(const graphlab::bitset<bool>& bitstr_in) : bitstr(bitstr_in) { }


    bit_str& operator+=(const bit_str& other) {
...
class vertex_program :
  public graphlab::ivertex_program<graph_type, msg_type, bit_str> {
...
public:
    ...
    void init(icontext_type& context, const vertex_type& vertex,
              const message_type& bit_accum) {
        updated = false;
        bitstring = bit_accum.bitstr;
    }
    ...
    msg_type gather(icontext_type& context, const vertex_type& vertex,
        ...
        for (size_t i = 0; i < NUM_SRC_NODES; i++) {
            gather_msg.ans[i] = other.data().ans[i];
        }
        return gather_msg;
    }
    ...
    void apply(icontext_type& context, vertex_type& vertex,
        ...
        for (size_t i = 0; i < NUM_SRC_NODES; i++) {
            if (bitstring.test_bit(i)) {
                if (!vertex.data().ans[i]) {
                    if (msg_accum.ans[i]) {
                        vertex.data().ans[i] = true;
                        updated = true;
                    }
                }   
            }
    ...
    void scatter(icontext_type& context, const vertex_type& vertex,
        ...
        const vertex_type other = get_other_vertex(edge, vertex);
        if (updated) {
            bit_str bit_msg = bit_str(bitstring);
            context.signal(other, bit_msg);
        }
    }
...
int main(int argc, char** argv) {
  ...
  graphlab::command_line_options 
    clopts("MultiLyra bBFS Algorithm.");
  ...
  for(size_t i = 0; i < sources.size(); ++i) {
    engine.signal_source(sources[i], bit_str(true, i));
  }
