...
struct msg_type {
    ...
    explicit msg_type(const graphlab::automi_bitvec<ans_type>& ans_in, const graphlab::automi_bitvec<bool>& track_in) : ans(ans_in), track(track_in) { }

    msg_type& operator+=(const msg_type& other) {
        graphlab::automi_bitvec<ans_type>::pair_op_or(ans, other.ans);
...
/**
 * \breif The vertex program class.
 */
...
public:
    vertex_program() {
        changed = graphlab::automi_bitvec<ans_type>(NUM_SRC_NODES);
    }
    ...
    void apply(icontext_type& context, vertex_type& vertex,
        ...
        changed.vec_op_andnot_update_mask(vp_track, vertex.data().ans, msg_accum.ans);
        vertex.data().ans.vec_op_or_update_mask(vp_track, msg_accum.ans);
    }
...
int main(int argc, char** argv) {
  ...
  for(size_t i = 0; i < sources.size(); ++i) {
    engine.signal(sources[i], msg_type(true, i));
  }
