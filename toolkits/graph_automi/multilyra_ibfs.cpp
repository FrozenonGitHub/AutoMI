 ...
 */
struct vertex_data {
    std::vector<ans_type> ans;
    std::vector<bool> status;
    
    vertex_data() {
        ans.resize(NUM_SRC_NODES);
        for (size_t i = 0; i < NUM_SRC_NODES; i++) {
            ans[i] = default_ans;
        }
        status.resize(NUM_SRC_NODES);
        for (size_t i = 0; i < NUM_SRC_NODES; i++) {
            status[i] = false;
        }
    }

    explicit vertex_data(const std::vector<ans_type>& ans_in, const std::vector<bool>& status_in) : ans(ans_in), status(status_in) { }
...
struct msg_type {
    ...
    msg_type(const std::vector<ans_type>& ans_in) {
        ans.resize(NUM_SRC_NODES);
        for (size_t i = 0; i < NUM_SRC_NODES; i++) {
            ans[i] = ans_in[i];
        }
    ...
    msg_type& operator+=(const msg_type& other) {
        for (size_t i = 0; i < NUM_SRC_NODES; i++) {
            ans[i] = std::min(ans[i], other.ans[i]);
        }
        return *this;
    }
...
/**
 * \brief struct bit_type
 */
struct bit_type {
    graphlab::bitset<bool> bitstr;

    bit_type(bool bitstr_init = false) {
        bitstr = graphlab::bitset<bool>(NUM_SRC_NODES);
        bitstr.set_all(bitstr_init);
    }

    // constructor for source vertices initialization
    bit_type(bool bitstr_init, size_t idx) {
        bitstr = graphlab::bitset<bool>(NUM_SRC_NODES);
        bitstr.set_all(false);
        bitstr.set_single(bitstr_init, idx);
    }

    // constructor with bitset array
    bit_type(const graphlab::bitset<bool>& bitstr_in) : bitstr(bitstr_in) { }


    bit_type& operator+=(const bit_type& other) {
        graphlab::bitset<bool>::or(bitstr, other.bitstr);
        return *this;
    }
    // serialization methods
    void save(graphlab::oarchive& oarc) const {
...
class vertex_program :
  public graphlab::ivertex_program<graph_type, msg_type, bit_type> {

    graphlab::bitset<bool> bitstring;
    bool updated;

public:
    vertex_program() {
        bitstring = graphlab::bitset<bool>(NUM_SRC_NODES);
        bitstring.set_all(false);
        updated = false;
    }

    // gather_nbrs function
    ...
    msg_type gather(icontext_type& context, const vertex_type& vertex,
        ...
        for (size_t i = 0; i < NUM_SRC_NODES; i++) {
            gather_msg.ans[i] = other.data().ans[i] + 1;
        }
        return gather_msg;
    }
    ...
    void apply(icontext_type& context, vertex_type& vertex,
               const msg_type& gather_accum) {
        updated = false;
        for (size_t i = 0; i < NUM_SRC_NODES; i++) {
            if (bitstring.test_bit(i)) {
                if (vertex.data().ans[i] > gather_accum.ans[i]) {
                    vertex.data().ans[i] = gather_accum.ans[i];
                    updated = true;
                }
            }
        }
    ...
    void scatter(icontext_type& context, const vertex_type& vertex,
        ...
        if (updated) {
            const vertex_type other = get_other_vertex(edge, vertex);
            bit_type scatter_msg = bit_type(bitstring);
            context.signal(other, scatter_msg);
        }
    }
...
int main(int argc, char** argv) {
  ...
  graphlab::command_line_options 
    clopts("MultiLyra iBFS Algorithm.");
  ...
  for(size_t i = 0; i < sources.size(); ++i) {
    engine.signal_source(sources[i], bit_type(true, i));
  }
