...
struct vertex_data : graphlab::IS_POD_TYPE {
    ...
    explicit vertex_data(const ans_type& ans_in, const bool& status_in) : ans(ans_in), status(status_in) { }
};
...
public:
    ...
    void apply(icontext_type& context, vertex_type& vertex,
        ...
        if (!vertex.data().status) {
          vertex.data().ans = msg_accum.ans;
          vertex.data().status = true;
          changed = true;
        }
    }
...
int main(int argc, char** argv) {
  ...
  graphlab::command_line_options 
    clopts("PowerLyra Closeness Centrality Algorithm.");
  std::string graph_dir;
  std::string query_dir;
  std::string format = "etuple";
  std::string exec_type = "plsync";
  size_t powerlaw = 0;
  ...
  for(size_t i = 0; i < sources.size(); ++i) {
    engine.signal_source(sources[i], msg_type(0));
    timer.start();
    engine.start();
