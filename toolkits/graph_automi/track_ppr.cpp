...
/**
 * \brief Parameter #sources for AutoMI vertex program
 */
int NUM_SRC_NODES;
float DAMP_FACTOR = 0.15;
int ITERATIONS = 10;

/**
 * \brief The type used to measure distances in the graph.
 */
typedef float ans_type;
ans_type default_ans = 0;
bool DIRECTED_GRAPH = true;

/**
 * \breif struct vertex_data for vertex property
 */
struct vertex_data {
    graphlab::automi_bitvec<ans_type> ans;
    graphlab::automi_bitvec<bool> identity;

    vertex_data() {
        ans = graphlab::automi_bitvec<ans_type>(NUM_SRC_NODES);
        ans.set_all(default_ans);
        identity = graphlab::automi_bitvec<bool>(NUM_SRC_NODES);
        identity.set_all(false);
    }

    explicit vertex_data(const graphlab::automi_bitvec<ans_type>& ans_in, graphlab::automi_bitvec<bool>& id_in) : ans(ans_in), identity(id_in) {}
...
struct msg_type {
    ...
    msg_type(int init_val, int idx) {
        ans = graphlab::automi_bitvec<ans_type>(NUM_SRC_NODES);
        ans.set_all(default_ans);
        ans.set_single(init_val, idx);
        track = graphlab::automi_bitvec<bool>(NUM_SRC_NODES);
        track.set_all(false);
        track.set_single(true, idx);
    }

    msg_type(const graphlab::automi_bitvec<ans_type>& ans_in, const graphlab::automi_bitvec<bool>& track_in) : ans(ans_in), track(track_in) { }

    // Gather\_Acc function
    msg_type& operator+=(const msg_type& other) {
        graphlab::automi_bitvec<ans_type>::pair_op_add(ans, other.ans);
...
public:
    ...
    msg_type gather(icontext_type& context, const vertex_type& vertex,
        ...
        msg.ans.vec_op_div_update(msg.ans, other.num_out_edges());
        return msg;
    }
    ...
    void apply(icontext_type& context, vertex_type& vertex,
        ...
        vertex.data().ans.vec_op_mul_update_mask(msg_accum.track, msg_accum.ans, DAMP_FACTOR);
        graphlab::automi_bitvec<bool> mask;
        mask.vec_op_set_mask(msg_accum.track, vertex.data().identity);
        vertex.data().ans.vec_op_add_update_mask(mask, 1.0 - DAMP_FACTOR);
    }
...
int main(int argc, char** argv) {
  ...
  graphlab::command_line_options 
    clopts("track personalized PageRank Algorithm.");
