...
struct vertex_data : graphlab::IS_POD_TYPE {
    ...
    explicit vertex_data(const ans_type& ans_in, const bool& id_in) : ans(ans_in), identity(id_in) { }
};
...
struct msg_type : graphlab::IS_POD_TYPE {
    ...
    explicit msg_type(const ans_type& ans_in) : ans(ans_in) { }
...
public:
    ...
    msg_type gather(icontext_type& context, const vertex_type& vertex,
        ...
        msg.ans = other.data().ans / other.num_out_edges();
        return msg;
    }
    ...
    void apply(icontext_type& context, vertex_type& vertex,
        ...
        vertex.data().ans = msg_accum.ans * DAMP_FACTOR;
        if (vertex.data().identity) {
            vertex.data().ans = vertex.data().ans + (1 - DAMP_FACTOR);
        }
    }
...
int main(int argc, char** argv) {
  ...
  graphlab::command_line_options 
    clopts("PowerLyra personalized Pagerank Algorithm.");
  ...
  dc.cout() << "----------------------------------------------------------"
            ...
            << std::endl
            << "`s executed: " << engine.num_updates() << std::endl
