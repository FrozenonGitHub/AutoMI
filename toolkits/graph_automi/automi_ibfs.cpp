...
struct vertex_data {
  ...
  graphlab::automi_bitvec<bool> status;

  vertex_data() {
    ans = graphlab::automi_bitvec<ans_type>(NUM_SRC_NODES);
    ans.set_all(std::numeric_limits<ans_type>::max());
    status = graphlab::automi_bitvec<bool>(NUM_SRC_NODES);
    status.set_all(false);
  }

  explicit vertex_data(const graphlab::automi_bitvec<ans_type>& ans_in, const graphlab::automi_bitvec<bool>& status_in) : ans(ans_in), status(status_in) {}
...
public:
    ...
    void apply(icontext_type& context, vertex_type& vertex,
        ...
        mask.vec_op_negate_mask(vp_track, vertex.data().status);
        vertex.data().ans.vec_op_set_mask(mask, msg_accum.ans);
        vertex.data().status.vec_op_set_mask(mask, true);
    ...
    void scatter(icontext_type& context, const vertex_type& vertex,
        ...
        msg.ans.vec_op_add_update_mask(mask, vertex.data().ans, 1);
...
int main(int argc, char** argv) {
  ...
  for(size_t i = 0; i < sources.size(); ++i) {
    engine.signal(sources[i], msg_type(0, i));
  }
