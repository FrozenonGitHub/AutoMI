...
struct msg_type {
  ...
  msg_type(const graphlab::automi_bitvec<color_type>& color_in, const graphlab::automi_bitvec<bool>& track_in) : color(color_in), track(track_in) { }
  /*
...
public:
  ...
  void apply(icontext_type& context, vertex_type& vertex,
    ...
    mask1.vec_op_cmpneq_update_mask(msg_acc.track, vertex.data().color, msg_acc.color);
  ...
  void scatter(icontext_type& context,
    ...
    if (!mask.vec_all_zeros()) {
      const vertex_type other = get_other_vertex(edge, vertex);
      msg_type msg(vertex.data().color, mask);
...
int main(int argc, char** argv) {
  ...
  graphlab::command_line_options 
    clopts("track Greedy Graph Coloring Algorithm.");
